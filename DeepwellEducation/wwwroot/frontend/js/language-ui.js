(function (global) {
    var LANGUAGE_DETAILS = {
        ...
        en: {
            name: 'English',
            country: 'United Kingdom, United States, Canada, Australia and more',
    ...
    function renderDetail(statusEl, articleEl, langRaw) {
        ...
        if (!info) {
            if (statusEl) statusEl.textContent = 'Language not found. Please select a language from Courses.';
        ...
        document.getElementById('lang-country-image').alt = info.name + ' country landmark';
        renderList(document.getElementById('lang-usage'), info.usage);
        renderList(document.getElementById('lang-country-facts'), info.countryFacts);
        document.getElementById('lang-courses-link').href = 'course.html?lang=' + encodeURIComponent(key);
        ...
        fetch('/api/Courses', { headers: { Accept: 'application/json' } })
            ...
            .then(function (courses) {
                var n = (courses || []).filter(function (c) {
                    return (c.languageCode || c.subjectCode || '').toLowerCase().trim() === langCode;
                }).length;
